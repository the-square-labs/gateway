...
| Pages | Project-based static-site hosting with immutable Deployments, mutable Tags (including system-managed `latest`), custom Routes that target Tags, optional wildcard previews, no-store runtime configuration, per-project node placement, and migration. Available on Personal and higher; project metadata and publication are available through AI Workspace and MCP, while artifact bytes use the resumable deploy API. |
| Docker | Container lifecycle, Business+ direct Git repository/branch push-to-deploy for containers and blue/green deployments, isolated Build Workers, a private-by-default Gateway-managed registry in every plan with optional Business+ external access, the Default (`runc`) runtime profile in every plan and the Secure (`runsc`/gVisor) profile in Business and Enterprise, Gateway-managed volumes, rollout/rollback, shared physical NVIDIA/AMD/Intel GPU attachment, eligible cross-node migrations, offline inventory snapshots, images, networks, tasks, webhooks, logs, console, files, secrets, env vars, ports, and cleanup. Secure workloads cannot use GPU, migration, or export; GPU-attached workloads cannot migrate or export in v1. |
...
| ![Business](docs/assets/license/wiolett-gw-business-24.png)<br>Business | $189 | $1,890 | Personal (including Pages) plus Git push-to-deploy with isolated Build Workers and build vulnerability policy, optional external access to the private internal registry, Docker Secure Runtime, structured logging, audit export, guided onboarding, and the in-development application cluster, same-node multi-instance, and Compose application features. |
