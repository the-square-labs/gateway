...
<table>
<tr>
<td align="center"><strong>Fleet overview</strong></td>
...
Gateway source owned by Alexandr Slavinschii, operating the project under the Square Labs name, is available under the [PolyForm Perimeter License 1.0.1](LICENSE.md). It permits use, modification, and redistribution for noncompeting purposes, including ordinary internal business use. The Square Labs OÜ becomes the rights holder only after registration and an effective written assignment. A Personal, Business, or Enterprise key unlocks the corresponding paid-plan features and limits under the [Paid Key Terms](COMMERCIAL-LICENSE.md); it does not waive Perimeter's restriction on providing a product marketed as a substitute for Gateway. Competing hosted/SaaS, OEM, white-label, and resale offerings require a separate written agreement with the Licensor.

Every official release also carries the [Product Continuity MIT Grant](CONTINUITY-MIT-GRANT.md). It has no release-age countdown and is not triggered by product inactivity, a support or sales pause, an announcement of discontinuation, or the unavailability of any website or licensing service. Before company formation and written assignment, no automatic transition applies. After assignment to The Square Labs OÜ, the conditional MIT terms can start only after final legal dissolution and 90 consecutive days without a successor or assignee, or through an express voluntary MIT election by the current rights holder. Trademarks, commercial keys, separately licensed documentation and assets, and third-party code are excluded.
...
After a paid key expires, full technical entitlements remain active for 24 hours on Personal, 3 days on Business, or 7 days on Enterprise. After that grace period, Gateway blocks creation of new paid-plan resources and other paid-only operations, but does not stop, delete, or disable already configured infrastructure. Existing services continue under the Paid Key Terms' continuity provisions. Revoked, replaced, invalid, or explicitly deactivated keys are handled separately and may disable protected entry points.

Copyright (c) 2021-2026 Alexandr Slavinschii, operating under the [Square Labs](https://thesquarelabs.com) project name
