...
| Pages | Project-based static-site hosting with immutable Deployments, mutable Tags (including system-managed `latest`), custom Routes that target Tags, optional wildcard previews, no-store runtime configuration, per-project node placement, and migration. Available on Personal and higher; project metadata and publication are available through AI Workspace and MCP, while remote MCP clients can also upload artifacts through an authenticated resumable tool without passing credentials as arguments. |
