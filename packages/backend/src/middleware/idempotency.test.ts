...
function createApp() {
  ...
  app.post('/api/pages-deploy/deployments', async (c) => {
    const n = count('pages');
    return c.json({ data: { attempt: n } }, 201);
  });
...
describe('Idempotency-Key middleware', () => {
  ...
  it('ignores the header on reads, excluded routes, and binary bodies', async () => {
    ...
    await app.request('/api/things', { headers: { 'x-test-token': 'token-1', ...key } });
    await post(app, '/api/pages-deploy/deployments', {}, key);
    await post(app, '/api/pages-deploy/deployments', {}, key);
    ...
    expect(calls).toMatchObject({ list: 2, pages: 2, upload: 2 });
  ...
  it('classifies eligible methods and excluded paths', () => {
    expect(isIdempotencyEligibleRequest('POST', '/api/domains')).toBe(true);
    expect(isIdempotencyEligibleRequest('put', '/api/proxy-hosts/1')).toBe(true);
    ...
    expect(isIdempotencyEligibleRequest('PUT', '/api/pages-deploy/uploads/u1/chunks')).toBe(false);
    expect(isIdempotencyEligibleRequest('POST', '/api/object-storage/s1/objects/upload')).toBe(false);
    expect(isIdempotencyEligibleRequest('POST', '/api/docker/nodes/n1/containers/archive')).toBe(false);
    expect(isIdempotencyEligibleRequest('PUT', '/api/nodes/n1/files/uploads/u1/chunks')).toBe(false);
  });
});
