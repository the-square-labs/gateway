...
import type { CreatedResourceDestination, CreatedResourceFamily } from './created-resource-scopes.js';
...
export async function grantCreatedResourcePermissions(
  ...
  await container.resolve(AuthService).grantCreatedResourcePermissions(userId, family, resourceId, destination);
}
