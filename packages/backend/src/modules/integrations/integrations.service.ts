...
export class IntegrationsService {
  ...
  private async upsertProjectRows(connectorId: string, projects: ProjectRowInput[], now = new Date()) {
  ...
  private toDockerBuildSourceRepository(connector: ConnectorRow, project: ProjectRow): DockerBuildSourceRepository {
    if (!['gitlab', 'github', 'git'].includes(connector.provider)) {
      throw new AppError(400, 'UNSUPPORTED_SOURCE_CONNECTOR', 'Connector does not provide a Git repository');
    }
    return {
