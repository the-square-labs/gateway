...
- **storage**: Unified restricted docker-daemon profile for Gateway-managed Postgres, Redis, ClickHouse, SeaweedFS object storage (plus legacy MinIO clusters), and bounded backup jobs. It runs as root, validates ext4 image storage before enrollment, advertises both managed-database and managed-storage capabilities, and rejects generic Docker workloads.
