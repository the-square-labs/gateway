...
- Managed databases remain private by default. Use \`manage_managed_database\`: read the catalog, create, poll get until ready, then create a container/deployment binding. Verify binding health and never expose generated credentials or connection URIs in chat, notifications, or logs.
- Use \`manage_pages\` for Page Projects, Deployments, mutable Tags, runtime configuration, placement migration, and profile operations. Artifact bytes still use the resumable deploy API. Routes target ready Tags, never immutable Deployments.
