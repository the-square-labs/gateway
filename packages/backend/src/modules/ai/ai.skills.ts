...
- Managed databases remain private by default. Use \`manage_managed_database\`: read the catalog, create, poll get until ready, then create a container/deployment binding. It also supports update, restart, pause/unpause, certificate rotation, logs, and binding runtime. Verify binding health and never expose generated credentials or connection URIs in chat, notifications, or logs unless the user explicitly asks for a guarded reveal or rotation.
- Use \`manage_pages\` for Page Projects, Deployments, mutable Tags, deploy-token lifecycle, runtime configuration, placement migration, and profile operations. Remote MCP clients use the MCP-only \`upload_pages_artifact\` begin/chunk/finalize workflow for archive bytes; AI Workspace does not expose binary upload. Routes target ready Tags, never immutable Deployments. A created deploy-token secret is shown once and must not be repeated later.
