...
Source in this package is published by Square Labs under the [PolyForm Perimeter License 1.0.1](LICENSE.md). A Personal, Business, or Enterprise Gateway key may unlock server-side Gateway Inference features and limits under the [paid access notice](COMMERCIAL-LICENSE.md), but an ordinary key does not waive Perimeter's restriction on providing a substitute product. The package also includes the [Product Continuity MIT Grant](CONTINUITY-MIT-GRANT.md).
