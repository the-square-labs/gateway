...
- Configuring a Docker or Pages build source needs `integrations:<provider>:use` on the repository (any qualifier covering it) next to the workload's own permissions; `repo:read`, an unqualified `view`, or a personal credential are not needed, because builds run with the connector credential. The source picker lists the connectors and repositories the caller's Git scopes cover (`use` implies `view`). A manual build checks the caller's `use` on the saved repository again. Sources saved before this release keep building automatically exactly as before. For a source created or saved again after the upgrade, polling, webhook and deferred builds run only while the account that saved it still holds `use` on the repository; a skipped build appears in the source's build history as "Build paused: <user> no longer has use on <repo>" and builds resume once that access returns or someone with `use` saves the source again.
- A GitLab project outside the caller's grant answers exactly like a project that is not synced (404, no path), so project IDs cannot be probed. Writes, secrets (CI/CD variables, webhooks, deploy tokens), sandbox clones and the connector-credential decision read group ancestry and repository ownership fresh from the provider; reads may use the cached value for up to five minutes. GitHub ownership is cached by repository ID, never by path. Connector syncs and source webhook deliveries drop the cached data.
- Connector change events over the WebSocket reach only callers who may see that connector.
- The scope picker reads `GET /api/integrations/{gitlab|github}/{connectorId}/scope-targets?search=&limit=50` (groups and projects, or owners and repositories) and `GET .../scope-targets/resolve?ids=group/123,project/456` (labels for stored qualifiers; `missing` when the target no longer exists). Both need `integrations:<provider>:view` on the connector or anything in it and return only targets the caller may view and the connector includes (its allowlist, unless it takes every visible repository); a stored qualifier the caller cannot view keeps its raw ID as the label. Picker requests are limited per account (60 a minute, then 429), and one request makes at most 60 uncached provider lookups; later targets stay unlabeled.
