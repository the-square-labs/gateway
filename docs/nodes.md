...
The wrapper is a compatibility loader: it downloads the matching
checksum-verified `gateway-installer` archive and forwards all arguments. The
archive embeds its Node runtime, so no system Node.js installation is needed.
After unpacking, the same command can be run directly with
`gateway-installer install node`.

Values already present in the copied command, including the Gateway address,
enrollment token, and TLS pin, are shown as editable defaults and are not
asked for again.
...
| `--nightly` | Opt in to the latest RC for this node's daemon type. |
...
## Daemon Release Candidates

Daemon RC tags use `vX.Y.Z-nginx-rc.N`, `vX.Y.Z-docker-rc.N`, or
`vX.Y.Z-monitoring-rc.N`. Because they do not end in a stable daemon suffix,
they are ignored by both current and already deployed Gateway daemon-update
checks. They are installed only when `--nightly` is passed to the setup loader.
Publish the matching Gateway RC (`vX.Y.Z-rc.N`) first: it provides the bundled
installer that the node loader downloads.

To install the latest matching daemon candidate on a test host:

```bash
curl -sSL https://gitlab.wiolett.net/wiolett/gateway/-/raw/main/scripts/setup-daemon.sh | \
  sudo bash -s -- --type nginx --nightly --gateway gw.example.com:9443 --token <TOKEN> --gateway-cert-sha256 sha256:<FINGERPRINT>
```

To pin a specific candidate, provide its complete daemon tag:

```bash
curl -sSL https://gitlab.wiolett.net/wiolett/gateway/-/raw/main/scripts/setup-daemon.sh | \
  sudo bash -s -- --type nginx --nightly --version v2.5.0-nginx-rc.1 --gateway gw.example.com:9443 --token <TOKEN> --gateway-cert-sha256 sha256:<FINGERPRINT>
```

For `databases`, use a Docker daemon candidate such as
`v2.5.0-docker-rc.1`.
...
For a `databases` node, the installer discovers writable disks on the target
host and asks where managed database images should be stored. Choose the disk
on that host; Gateway does not need to know it when generating the enrollment
command. In unattended mode, pass `--storage-root /absolute/path`.

The installer validates the selected path and performs the ext4 image storage
preflight before enrollment, so an unusable disk cannot create a pending node.
