...
- Git-source builds run only on nodes enrolled as Build Workers. The same `docker-daemon` binary runs in a command-restricted `builder` mode with no Docker Engine socket; it supervises a dedicated BuildKit containerd worker and dedicated containerd configured for the `runsc` shim. Build admission fails closed unless the worker advertises execution, runsc, and enforced-resource-profile capabilities.
